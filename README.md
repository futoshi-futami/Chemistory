...
このリポジトリは、`GPR_handoff.zip` の3つのCSVを使い、受領したRFに対応するGaussian Process Regression（GPR）を複数カーネルで比較する主解析です。固定10-foldの系列内補間だけでなく、暫定trajectoryを丸ごとHeld outしたRF/GPR比較まで収録します。
...
| 結果 | 固定10-foldの相互作用GPは `R²=0.973807`、RF報告値は0.908223。trajectory-group5では同GP 0.465545、受領二段RF 0.025996 |
...
ここで元のbaseは111列です。ただしProduct GPでは、重複する方向7列を4個の分子軸特徴へ置換し、残る104列から訓練fold内で分散ゼロの2列を除いた102列を環境baseにします。環境 `e` はこの102列＋fold内 `X_proc` PCA8＝110次元、軸 `a` は4次元、入力全体は114次元です。4列の名前、除外列、数式は[カーネル文書 7.2](docs/KERNELS_JA.md#72-関数形)に明記しています。
...
| 同上 | 受領二段R RF | −0.212622 | 11.151589 | 5.720765 |
...
| 同じ外側group5、候補を相互作用GPに固定 | 相互作用GP | **0.465545** | **7.403375** | **4.513588** |
| 同じ外側group5 | 受領二段R RF | 0.025996 | 9.994348 | 5.451921 |
...
![分子軸の方向ベクトル模式図、Product GPの条件付き予測平均面、予測標準偏差面](figures/gpr_handoff_static_overview.png)

左は受領summaryから復元できる方向ベクトル模式図、中央・右は他特徴を基準試料に固定した条件付きsliceです。左は元原子構造ではなく、中央・右も性能評価用OOF面ではありません。
...
- [分子軸trajectory模式図＋OOF予測区間](figures/gpr_handoff_molecular_axis_uncertainty_animation.html)
- [分子軸方位×面外傾きの3D予測平均・GP分散](figures/gpr_handoff_interaction_surface_axis_tilt.html)
- [分子軸方位×H3近傍環境の3D予測平均・GP分散](figures/gpr_handoff_interaction_surface_h3_environment.html)
- [全170 OOF点の予測区間・分散–誤差診断](figures/gpr_handoff_oof_uncertainty.html)
...
この旧screenへ新モデルを同じprefix-group10で加えると、相互作用GPが `R²=0.365228, RMSE=8.068315` へ改善し、40°gate Matérn 3/2→1/2は `R²=0.275269` でした。改善は明瞭ですが、固定foldの `R²=0.973807` との差は大きく、「未知の分子構造系列への外挿」は未解決です。受領二段R RFも同じgroup5/group10で再fitし、`results/gpr_handoff_trajectory_model_comparison.csv` へ直接比較を保存しています。

### trajectory Held-outでの受領RFとProduct GP

受領したR実装を変更せず、fold表だけを暫定trajectory単位へ置き換えました。`RF_R_base` は111 base特徴の1,000-tree RF、`RF_R_base_plus_residualPLS5` はそのOOB残差を `X_proc` PLS5＋第2の1,000-tree RFで補正する最終モデルです。

| 暫定trajectory分割 | base RF R² / RMSE | 二段RF R² / RMSE | Product GP R² / RMSE |
| --- | ---: | ---: | ---: |
| group5 | 0.073894 / 9.745507 | 0.025996 / 9.994348 | **0.465545 / 7.403375** |
| group10 | −0.046150 / 10.357884 | −0.212622 / 11.151589 | **0.365228 / 8.068315** |

残差PLS5補正は固定10-foldではbase RFを `R²=0.853187` から0.900572へ改善しますが、trajectory Held-outでは両方の分割で悪化しました。補正段が同じtrajectory内では再利用できる細かな残差構造を学ぶ一方、未知trajectoryへは移らない可能性があります。Product GPも固定foldから大幅に低下するため外挿問題自体は未解決ですが、同じ外側groupでは受領RFより明瞭に良好でした。

分子軸方位帯別RMSEでは、Product GPがgroup10の5/5帯、group5の4/5帯で受領二段RFを上回りました。例外はgroup5の−30–−10°で、RF 6.318、GP 6.855です。ただし同帯のMAEはGP 2.581、RF 4.181であり、RFのRMSE優位は少数の大誤差の配置に依存します。50–70°はgroup5でGP 11.561 / RF 14.717、group10でGP 12.340 / RF 15.666と、両者にとって最難関のままです。全表は `results/gpr_handoff_trajectory_angle_model_comparison.csv` にあります。
...
この1冊が本研究の中心です。必要ならColabへR本体と `randomForest`, `pls` を導入し、Pythonから `Rscript` を呼びます。ローカルWindows用の `Rscript.exe` パス指定は不要です。
...
python scripts/run_rf_trajectory.py         # 同じ受領二段RFをtrajectory group5/group10で評価
...
python scripts/build_static_overview.py        # GitHub README用の静的3面プレビュー
pytest -q
```
...
- `results/gpr_handoff_rf_trajectory_metrics.csv`: 受領二段R RFのtrajectory group5/group10性能
- `results/gpr_handoff_rf_trajectory_angle_metrics.csv`: trajectory Held-out RFの分子軸角度帯別性能
- `results/gpr_handoff_trajectory_model_comparison.csv`: 同じtrajectory splitでの受領RFとProduct GP比較
