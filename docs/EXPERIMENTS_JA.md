...
生の `C3H3_angle_xy` は−179.83°から178.15°にまたがるため、通常の数直線上でbin分けすると、同じ方向に近い−180°と+180°を別群にしてしまいます。また、C3→H3とC6→H6のxy方向は全170例でほぼ反対向きでした。入力 `angle_diff_C3_C6` は176.99–179.98°で、反平行からのずれは平均0.374°、最大3.012°にすぎません。
...
6. **元構造へ戻れる場合の回転不変特徴:** 分子軸に沿ったMg/O位置の射影、軸からの垂直距離、H3/H6側の符号付き非対称性を作ります。絶対方位そのものより「分子軸に対して原子がどこにあるか」を表すため、物理解釈と別座標系への一般化が改善します。
