...
未知系列では、多尺度のRational Quadraticと粗いMatérn 1/2が滑らかなMatérn 3/2・5/2・RBFより相対的に良く、角度帯診断と同じく非定常性・急なregime変化を示唆します。ただし最良でもR² 0.205なので、カーネル交換だけでは構造外挿を解決できません。また、RのRFはこのgroup分割で再実行していないため、group評価からRF対GPRの優劣は述べません。
