...
- 同じ特徴表現の等方RBFは `R2=0.933505` と僅差でしたが、RBF-ARDは `R2=0.872676` へ低下したため、`GPR_handoff` の最高性能モデルは変わりません。
- 受領コードを再実行したRF + residual PLS5の `R2=0.900572` より良好でした。ただし、GPR候補の選択にも同じ10-fold結果を使っているため、最終的な優劣の確定には独立テストデータまたはnested CVが必要です。
- `dist_auto` のtag 10では、元ノートブックと同じRBF-ARD + White noiseが `R2=0.993259` となり、previous Matérn 3/2の `0.978301` を上回りました。ただしRBF-ARDの95% coverageは0.745で、不確実性区間は過度に狭い結果です。
- tag 10, 15, 20, 25への外挿は高精度ですが、tag a、とくにtag bへの外挿は難しく、構造領域をまたぐ汎化性能は一様ではありません。
...
公平なカーネル比較では、角度のsin/cos周期表現とfold内 `X_proc` PCA8を共通にし、次の4種類だけを変更しました。
...
| `base_cyclic_xproc_pca8_rbf_ard_original` | RBF / Squared Exponential | 特徴ごとのARD |
...
| GPR: 周期角度、PCA8、Matérn 3/2 | **0.933874** | **2.604119** | 1.191774 | 0.952941 | 13.768523 | 2.470919 |
| GPR: 周期角度、PCA8、等方RBF | 0.933505 | 2.611376 | 1.217731 | 0.935294 | 12.774825 | **2.425369** |
| GPR: 周期角度、PCA8、Matérn 1/2 | 0.926884 | 2.738293 | **1.176260** | 0.952941 | 14.951085 | 2.484521 |
| GPR: 周期角度、PCA8、RBF-ARD | 0.872676 | 3.613510 | 1.264649 | 0.876471 | 8.015844 | 5.221269 |
...
2. **カーネルを揃えた比較で、最高R²はMatérn 3/2のままです。** 等方RBFとの差は0.000369だけなので、データだけから両関数族を強く区別できるわけではありません。
...
4. RBF-ARDは全特徴のまま増やさず、fold内の定数列除去・重要特徴量選択・PLS/PCA圧縮と組み合わせて再評価する。
5. RBF-ARDの予測区間をheld-out tagで較正し、必要ならconformal calibrationを比較する。
6. tag bの構造差を可視化し、幾何特徴、regime別GPR、階層／multi-task GPをdomain-awareな分割で評価する。

対応する実装は `src/chemistory_gpr/kernels.py`、`handoff.py`、`dist_auto.py`、実行結果は `results/gpr_handoff_metrics.csv` と `results/dist_auto_kernel_comparison_metrics.csv` にあります。
