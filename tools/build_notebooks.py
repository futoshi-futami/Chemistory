...
def notebook_one() -> nbf.NotebookNode:
    ...
    nb["cells"] = [
        ...
        nbf.v4.new_markdown_cell(
            ...
            "| 前処理 | 方向7列を4軸特徴へ整理。`X_proc` は各訓練fold内でPCA8へ圧縮 |\n"
            "| 評価 | 固定10-foldは既知系列内補間、trajectory group CVは未知系列外挿 |\n"
            "| 主結果 | 固定10-fold: Product GP R² 0.973807、RF 0.908223。trajectory group5: GP 0.465545、RF 0.025996。strict nested: R² 0.334491 |\n"
            "| 結論 | 軸×環境GPは有望だが、未知trajectory外挿は未解決 |\n\n"
            "性能はOOFまたは外側groupのHeld-out予測から計算します。3D面は性能図ではなく条件付き断面です。"""
        ),
        nbf.v4.new_markdown_cell(
            r"""## 比較するカーネル

標準化後の距離を $r$ とすると、各信号カーネルは次の形です。すべてに $k_W=\sigma_n^2\mathbf 1[x=x']$ を加えます。

| 名前 | 数式 |
| --- | --- |
| Matérn 1/2（Exponential） | $k_{M12}=\sigma_f^2e^{-r}$ |
| Matérn 3/2 | $k_{M32}=\sigma_f^2(1+\sqrt3r)e^{-\sqrt3r}$ |
| Matérn 5/2 | $k_{M52}=\sigma_f^2(1+\sqrt5r+5r^2/3)e^{-\sqrt5r}$ |
| RBF | $k_{RBF}=\sigma_f^2e^{-r^2/2}$ |
| Rational Quadratic | $k_{RQ}=\sigma_f^2(1+r^2/(2\alpha))^{-\alpha}$ |
| Linear | $k_{Lin}=\sigma_f^2(\sigma_0^2+x^\top x')$ |

主モデルは $k_{Product}=k_a+k_e+k_{a\times e}+k_W$ です。$k_a$ と $k_e$ はMatérn 3/2、$k_{a\times e}$ は軸Matérn 3/2と環境Matérn 3/2の積です。"""
        ),
        ...
        nbf.v4.new_code_cell(
            ...
            "print('角度帯ごとのRF/Product GP比較（両モデルのRMSE）')\n"
            "trajectory_angle_table = trajectory_angle_comparison.pivot_table(\n"
            "    index=['evaluation_split','angle_bin','n'], columns='method', values='RMSE', aggfunc='first'\n"
            ").reset_index().rename(columns={\n"
            "    'Product_Matern32_GP': 'Product_GP_RMSE',\n"
            "    'received_two_stage_R_RF': 'received_RF_RMSE',\n"
            "})\n"
            "trajectory_angle_table['angle_bin'] = pd.Categorical(trajectory_angle_table['angle_bin'], bin_order, ordered=True)\n"
            "trajectory_angle_table = trajectory_angle_table.sort_values(['evaluation_split','angle_bin'])\n"
            "display(trajectory_angle_table)\n"
        ...
        nbf.v4.new_markdown_cell(
            ...
            "ほぼ反平行なC3→H3/C6→H6の方向7列を、方位sin、方位cos、面外傾きproxy、反平行ずれの4軸特徴 `a` へ整理します。"
            "周期境界を正しく扱い、重複を減らし、PowerPointと残差解析で示唆された面内方位・`z≈0`・歪みを残すためです。\n\n"
            "環境 `e` は、元base 111列−方向7列−fold内定数2列＝102列と、fold内 `X_proc` PCA8の合計110次元です。"
            "Product GP入力は `a` 4＋`e` 110＝114次元です。\n\n"
            "`k_axis + k_environment + k_axis×k_environment + White` の積項は、軸も環境も似た試料だけに強い共分散を与えます。"
            "通常Matérn 3/2の誤差が高角度へ集中し、同じ角度でもH3近傍Mg/Oと面外傾きで応答が枝分かれしたため導入しました。"
        ...
        nbf.v4.new_code_cell(
            ...
            "primary_comparison = pd.read_csv(RESULTS / 'gpr_handoff_primary_comparison.csv')\n"
            "trajectory_comparison = pd.read_csv(RESULTS / 'gpr_handoff_trajectory_model_comparison.csv')\n"
            "fixed_product = fixed_next.loc[fixed_next['candidate'].eq('axis_environment_interaction_matern32')].iloc[0]\n"
            "reported_rf = primary_comparison.loc[primary_comparison['source'].eq('RF_reported_reference')].iloc[0]\n"
            "best_rows = [{\n"
            "    'Held-out': 'fixed10: known-trajectory interpolation',\n"
            "    'Product_GP_R2': fixed_product['R2'], 'Product_GP_RMSE': fixed_product['RMSE'],\n"
            "    'received_RF_R2': reported_rf['R2'], 'received_RF_RMSE': reported_rf['RMSE'],\n"
            "}]\n"
            "for split in ['trajectory_group5', 'trajectory_group10']:\n"
            "    rows = trajectory_comparison.loc[trajectory_comparison['evaluation_split'].eq(split)]\n"
            "    gp = rows.loc[rows['model'].eq('axis_environment_interaction_matern32')].iloc[0]\n"
            "    rf = rows.loc[rows['model'].eq('RF_R_base_plus_residualPLS5')].iloc[0]\n"
            "    best_rows.append({\n"
            "        'Held-out': split, 'Product_GP_R2': gp['R2'], 'Product_GP_RMSE': gp['RMSE'],\n"
            "        'received_RF_R2': rf['R2'], 'received_RF_RMSE': rf['RMSE'],\n"
            "    })\n"
            "print('現在のBestと同じsplitの受領RF')\n"
        ...
        nbf.v4.new_code_cell(
            ...
            "standard_angle = pd.read_csv(RESULTS / 'gpr_handoff_angle_method_metrics.csv')\n"
            "axis_standard = standard_angle.loc[\n"
            "    standard_angle['angle_view'].eq('molecular_axis') & standard_angle['method_type'].eq('GPR')\n"
            "].copy()\n"
            "best_standard = axis_standard.loc[axis_standard.groupby('angle_bin')['RMSE'].idxmin(), ['angle_bin','model','RMSE']].rename(\n"
            "    columns={'model':'best_standard_GPR','RMSE':'best_standard_GPR_RMSE'}\n"
            ")\n"
            "product_angle = fixed_angle.loc[\n"
            "    fixed_angle['candidate'].eq('axis_environment_interaction_matern32'), ['angle_bin','n','RMSE','MAE']\n"
            "].rename(columns={'RMSE':'Product_GP_RMSE','MAE':'Product_GP_MAE'})\n"
            "rf_angle = standard_angle.loc[\n"
            "    standard_angle['angle_view'].eq('molecular_axis') & standard_angle['model'].eq('RF_current_residualPLS5'),\n"
            "    ['angle_bin','RMSE'],\n"
            "].rename(columns={'RMSE':'RF_current_rerun_RMSE'})\n"
            "fixed_angle_table = product_angle.merge(best_standard, on='angle_bin').merge(rf_angle, on='angle_bin')\n"
            "fixed_angle_table['angle_bin'] = pd.Categorical(fixed_angle_table['angle_bin'], bin_order, ordered=True)\n"
            "fixed_angle_table = fixed_angle_table.sort_values('angle_bin')\n"
            "print('固定10-foldの角度帯別結果')\n"
            "display(fixed_angle_table)\n"
            "selected = {\n"
        ...
        nbf.v4.new_markdown_cell(
            ...
            "残り110入力が異なる観測点はこの面上に乗る必要がないため、更新後は比較可能な基準試料1点だけを重ねます。"
            "White noiseを含むので、その1点でも予測平均が観測値と完全一致する必要はありません。"
            "フィット具合はH.3のOOF実測対予測図で判断します。分散が大きい領域の平均値は強く解釈しないでください。"""
        ),
        nbf.v4.new_code_cell(
